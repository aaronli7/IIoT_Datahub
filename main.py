...
X_organized, Y_organized = sliding_windows(X.flatten(), 2, 1, 0)
...
# plt.plot(y_test.squeeze() - test_preds.detach().numpy().squeeze())
plt.plot(y_test.squeeze())
plt.show()
exit()

diff = y_test.squeeze() - test_preds.detach().numpy().squeeze()
